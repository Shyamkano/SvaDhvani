...
const AnimatedWave = ({ top, duration, direction = 'forward' }: { top: string, duration: number, direction?: 'forward' | 'backward' }) => {
  ...
  // Convert percentage string to number value for 'top'
  let topValue: number = 0;
  if (top.endsWith('%')) {
    const percent = parseFloat(top) / 100;
    topValue = percent * height;
  } else {
    topValue = Number(top);
  }

  return (
    <Animated.View style={[{ position: 'absolute', top: topValue, left: 0 }, animatedStyle]}>
...
export default function SplashScreen() {
  ...
  // Navigate away from the splash screen after a delay
  useEffect(() => {
    const timer = setTimeout(() => {
      router.replace('/(tabs)'); // Navigate to your main tab layout
    }, 4000); // 4 seconds
...
const styles = StyleSheet.create({
  ...
  logoGlow: {
    ...
    borderRadius: 64,
    opacity: 0.4,
    // Note: Blur effect is difficult to replicate perfectly, this glow is a native-friendly alternative
  },
  logoGradient: {
    width: '100%',
    height: '100%',
